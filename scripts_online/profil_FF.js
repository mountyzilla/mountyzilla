...
function resiste(x,y) {
	...
	return Math.round(val1 * 100) / 100 + degbmm/2+y/2;
}
...
function saveProfil()
{
	...
	MZ_setValue(numTroll+".caracs.pv",pvactuels);
	MZ_setValue(numTroll+".caracs.pv.max",pvtotal);
	MZ_setValue(numTroll+".caracs.rm",rm);
	MZ_setValue(numTroll+".caracs.rm.bm",rmbonus);
	MZ_setValue(numTroll+".caracs.mm",mm);
...
function init() {
	...
	posX = parseInt(a[0].substring(4, a[0].length - 1));
	posY = parseInt(a[1].substring(5, a[1].length - 1));
	posN = parseInt(a[2].substring(5, a[2].length - 1));
	...
	appendTdText(arrTable[3].childNodes[1].childNodes[10].childNodes[3].childNodes[0].childNodes[1].childNodes[0],
	 '(moyenne : ' + regmoy + ')',false);
//	arrTable[3].childNodes[1].childNodes[10].childNodes[3].childNodes[0].childNodes[1].childNodes[0].childNodes[10].setAttribute('width', '50%');
	...
	arm = listeValeurs['arm'][0];
	if(listeValeurs['arm'].length==3)
	{
		armbonus = listeValeurs['arm'][1];
		armbmm = listeValeurs['arm'][2];
	}
	else
	{
		armbonus = listeValeurs['arm'][2];
		armbmm = listeValeurs['arm'][3];
	}
		
	armmoy = arm * 2 + armbonus+armbmm;
	
	appendTdText(arrTable[3].childNodes[1].childNodes[10].childNodes[3].childNodes[0].childNodes[1].childNodes[8],'(moyenne : ' + armmoy + ')',false);
...
function setLieu() {
	arrTR[4].childNodes[5].setAttribute('rowspan',12);
...
function setFatigue() {
	...
	var race = document.evaluate("//tr/td/text()[contains(.,'Race...')]",
			arrTable[3], null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.nodeValue;
...
function setRatioKillDeath() {
	var node = document.evaluate("//tr/td/p[contains(./text(),'Adversaires tués')]",
			arrTable[3], null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
...
function setCurrentEsquive() {
	...
	nbattaques = 1*esqmod;
	nodes = document.evaluate("//p/text()[contains(.,'armure en moins')]",
			document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
	var armmod = nodes.snapshotItem(0).nodeValue;
	armmod = Math.abs(armmod.substring(armmod.indexOf(': ') + 2, armmod.length));
	nodes.snapshotItem(0).nodeValue += ' (Moyenne armure : ' + Math.max(armmoy - 2 * armmod, armbonus+armbmm) + ')';
}
...
// Sauvegarde quelques infos sur le troll pour être utilisées depuis d'autres pages
...
function competences(comp, niveau) {
	...
	if(comp.indexOf('Acceleration du Metabolisme') != -1 && minuteFatigue != null)
		...
		var corpulence = Math.floor(pvtotal/10)+arm;
		texte = 'Attaque : <b>' + Math.floor((reg+esq) / 2) + '</b> D6 '+ aff(attbonus+attbmm);
		texte += ' => <b>' + ((Math.floor((reg+esq) / 2) * 7 / 2)) + '</b><br/>';
		texte += 'Effet : <b>Mets à terre l\'adversaire</b>';
...
function sortileges(sort) {
	...
	if (sort.indexOf('Analyse Anatomique') != -1) {
		...
		texte += ' (' + resiste(deg / 2,degbmm) + '/' + resiste(Math.floor(deg / 2) * 1.5,degbmm) + ')</b></br>';
		...
		texte += resiste(vue / 2,degbmm) + '/';
		texte += resiste(Math.floor(vue / 2) * 1.5,degbmm) + ')</b>';
		...
		texte += ' => <b>' + (deg * 2 + degbmm) + ' (' + resiste(deg,degbmm) + ')</b><br/>';
		...
		texte = 'Attaque : <b>' + att + '</b> D6 '+aff(attbmm);
		texte += ' => <b>' + (Math.floor(att * 3.5)+attbmm) + '</b>';
		texte += '<br/>Dégâts : <b>' + reg + '</b> D3 '+aff(degbmm);
		texte += ' => <b>' + (reg * 2 + degbmm) + ' (' + resiste(reg,degbmm) + ')</b><br/>';
		texte += 'Attaque/Dégâts : <b>-' + reg + '</b>';
		...
		texte += ' => <b>' + (deg * 2 + degbmm) + '/' + (Math.floor(deg * 1.5) * 2 + degbmm) + ' ('
				+ resiste(deg,degbmm) + '/' + resiste(deg * 1.5,degbmm) + ')</b>';
